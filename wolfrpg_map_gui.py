...
class WolfMapGUI:
    def __init__(self, root):
        self.root = root
        root.title("WOLF RPG 地图导出工具v1.0")
    ...
    def _build_ui(self):
        ...
        tk.Button(top, text="选择游戏(Data 文件夹)", command=self.select_game).pack(side=tk.LEFT)
        self.dir_lbl = tk.Label(top, text="未选择", anchor='w', fg='#555')
        ...
        hdr = tk.Frame(left)
        hdr.pack(fill=tk.X)
        tk.Label(hdr, text="地图列表（勾选=导出）:").pack(side=tk.LEFT)
        tk.Button(hdr, text="全选", command=self.select_all).pack(side=tk.LEFT, padx=2)
        tk.Button(hdr, text="反选", command=self.invert_selection).pack(side=tk.LEFT, padx=2)
        ...
        for text, val in [("事件层（点事件切换显隐）", "events"), ("地图层", "map"), ("两者", "both")]:
            tk.Radiobutton(bottom, text=text, variable=self.export_mode, value=val,
                           command=self.render_preview).pack(side=tk.LEFT)
        tk.Button(bottom, text="导出当前地图", command=self.export_current_map).pack(side=tk.LEFT)
        tk.Button(bottom, text="导出所选地图", command=self.export_map).pack(side=tk.LEFT, padx=8)
        tk.Button(bottom, text="复制到剪贴板", command=self.copy_to_clipboard).pack(side=tk.LEFT)
    ...
    def select_game(self):
        ...
        try:
            d = filedialog.askdirectory(initialdir=initial, title="选择游戏 Data 文件夹")
        except Exception:
            d = filedialog.askdirectory(title="选择游戏 Data 文件夹")
        if not d:
            return
        _save_last_dir(d)
        cand = Path(d)
    ...
    def _require_data_root(self):
        if not self.data_root or not Path(self.data_root).exists():
            messagebox.showinfo("提示", "请先选择游戏（Data 文件夹）")
            return False
        return True
    ...
    def export_current_map(self):
        ...
        if not self.map_path or not self.map_path.exists():
            messagebox.showinfo("提示", "请先选择一张地图")
            return
        mode = self.export_mode.get()
    ...
    def export_map(self):
        ...
        if not checked:
            messagebox.showinfo("提示", "请先勾选要导出的地图")
            return
    ...
    def copy_to_clipboard(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showinfo("提示", "已复制到剪贴板（可粘贴到 Word 或文件夹）")
            except Exception as ce:
                messagebox.showerror("剪贴板失败", str(ce))
        except Exception as e:
